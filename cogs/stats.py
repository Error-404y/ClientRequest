...
class Stats(commands.Cog):
    ...
    async def stats(self, ctx, member: discord.Member = None):
        ...
        log_command(
            ...
            "!stats",
            ctx.channel,
            details=f"target={member.display_name if member else 'self'}",
        )
        ...
        embed.add_field(
            name="Tickets Claimed", value=f" `{claimed_count}`", inline=True
        )
        embed.add_field(name="Tickets Closed", value=f" `{closed_count}`", inline=True)
        embed.add_field(
            name="Avg. Claim Response", value=f" `{avg_claim_time}`", inline=False
        )
        embed.add_field(
            name="Avg. Resolution Time", value=f" `{avg_resolve_time}`", inline=False
        )
    ...
    async def leaderboard(self, ctx):
        ...
        closures_str = (
            "\n".join(closures_list)
            if closures_list
            else "*No closure data available.*"
        )
