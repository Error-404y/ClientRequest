...
class UUIDLookup(commands.Cog):
    ...
    async def get_infraction(
        ...
        uuid_value = self.normalize_uuid(uuid_value)
    ...
    @app_commands.command(
        name="findz", description=("Find information connected to a generated UUID.")
    )
    @app_commands.describe(uuid=("The ticket or infraction UUID you want to look up."))
    async def findz(self, interaction: discord.Interaction, uuid: str):
        ...
        if infraction:
            ...
            if str(infraction["action_type"]).upper() == "WARN":
                description = (
            ...
            if infraction["guild_id"]:
                embed.add_field(
                    name="Guild ID", value=str(infraction["guild_id"]), inline=True
                )
        ...
        if ticket:
            ...
            if ticket["guild_id"]:
                embed.add_field(
                    name="Guild ID", value=str(ticket["guild_id"]), inline=True
                )

            if ticket["channel_id"]:
                embed.add_field(
                    name="Channel ID", value=str(ticket["channel_id"]), inline=True
                )
