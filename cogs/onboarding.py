...
def parse_ticket_options(value):
    ...
    for item in value.split(","):
        ...
        if 2 <= len(cleaned) <= 50 and cleaned.casefold() not in {
            entry.casefold() for entry in options
        }:
...
def resource_report(guild, settings):
    ...
    resources = {
        ...
        "Ticket panel channel": (
            settings.get("TICKET_PANEL_CHANNEL_ID"),
            discord.TextChannel,
        ),
    ...
    if isinstance(panel_channel, discord.TextChannel):
        ...
        if (
            panel_channel.name != "ticket"
            or panel_category is None
            or panel_category.name != "ticket-system"
        ):
            issues.append(
                "Ticket panel must be #ticket inside the ticket-system category"
            )
...
class Onboarding(commands.Cog):
    setup_group = app_commands.Group(
        name="setup", description="Configure the ticket system for this server"
    )
    add_group = app_commands.Group(
        name="add", description="Add access to the ticket system"
    )
    remove_group = app_commands.Group(
        name="remove", description="Remove access from the ticket system"
    )
    ...
    async def welcome_channel(self, guild):
        ...
        candidates.extend(
            channel for channel in guild.text_channels if channel not in candidates
        )
    ...
    async def deliver_welcome(self, guild):
        ...
        try:
            ...
            log_exception(
                ...
                error,
                guild=guild,
                context="Welcome message delivery failed",
            )
    ...
    async def on_ready(self):
        ...
        for guild in self.bot.guilds:
            if guild.id not in config.GUILDS or not config.GUILDS[guild.id].get(
                "WELCOME_SENT"
            ):
                await self.deliver_welcome(guild)
    ...
    async def resolve_staff_role(self, guild, requested_role, current_settings):
        ...
        existing = discord.utils.find(
            lambda role: role.name.casefold() == f"{config.BOT_NAME} staff".casefold(),
            guild.roles,
        )
        if existing:
            return existing
    ...
    async def ensure_category(self, guild, channel_id, name, overwrites):
        ...
        if isinstance(channel, discord.CategoryChannel):
            return channel
        existing = discord.utils.find(
            lambda item: item.name.casefold() == name.casefold(), guild.categories
        )
        if existing:
            return existing
    ...
    async def ensure_text_channel(
        self, guild, channel_id, name, category, overwrites, topic
    ):
        channel = guild.get_channel(channel_id) if channel_id else None
    ...
    def is_ticket_panel_message(self, message):
        return any(
            getattr(component, "custom_id", None) == "zer_application_dropdown"
            for row in message.components
            for component in getattr(row, "children", [])
        )

    async def publish_ticket_panel(self, guild, panel_channel, settings):
    ...
    async def configure_server(self, guild, staff_role, ticket_types):
        current = dict(
            ...
            or config.normalize_guild_config(guild.id, {"NAME": guild.name})
        )
        requested_role = (
            staff_role if staff_role and staff_role != guild.default_role else None
        )
        role_will_be_created = (
            requested_role is None
            and guild.get_role(current.get("MOD_ROLE", 0)) is None
        )
        missing_permissions = setup_permission_report(
            guild, needs_role_creation=role_will_be_created
        )
        ...
        await ticket_category.set_permissions(
            guild.default_role, view_channel=False, reason=f"{config.BOT_NAME} setup"
        )
        ...
        if (
            panel_channel.name != "ticket"
            or panel_channel.category_id != panel_category.id
        ):
        ...
        current.update(
            {
                ...
                "TICKET_OPTIONS": parse_ticket_options(ticket_types)
                if ticket_types is not None
                else current.get("TICKET_OPTIONS", config.DEFAULT_TICKET_OPTIONS),
                "SETUP_COMPLETE": True,
                "WELCOME_SENT": True,
            }
        )
        ...
        await self.publish_ticket_panel(guild, panel_channel, settings)
        return settings, []

    @setup_group.command(
        name="start", description="Automatically configure the complete ticket system"
    )
    ...
    async def setup_start(
        ...
        try:
            settings, missing_permissions = await self.configure_server(
                interaction.guild, staff_role, ticket_types
            )
        ...
        log_interaction(
            ...
            "setup start",
            interaction.channel,
            details=f"Guild: {interaction.guild.id}",
        )
        embed = discord.Embed(
        ...
        embed.add_field(
            name="Ticket Panel",
            value=f"<#{settings['TICKET_PANEL_CHANNEL_ID']}>",
            inline=True,
        )
        embed.add_field(
            name="Ticket Category",
            value=f"<#{settings['TICKET_CATEGORY_ID']}>",
            inline=True,
        )
        embed.add_field(
            name="Archive Category",
            value=f"<#{settings['TICKET_ARCHIVE_CATEGORY_ID']}>",
            inline=True,
        )
        embed.add_field(
        ...
        embed.add_field(
            name="Staff Role", value=f"<@&{settings['MOD_ROLE']}>", inline=True
        )
        embed.add_field(
            name="Ticket Types",
    ...
    @setup_group.command(
        name="status", description="Check configuration and required permissions"
    )
    async def setup_status(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="Setup Status",
            color=0x2ECC71
            if not missing_permissions and not resource_issues
            else 0xF0B232,
        )
        embed.add_field(
            name="Configuration",
        ...
        embed.add_field(
            name="Permissions",
            value="Ready" if not missing_permissions else "Attention required",
            inline=True,
        )
        embed.add_field(
            name="Resources",
            value="Ready" if not resource_issues else "Attention required",
            inline=True,
        )
        if settings:
            embed.add_field(
            ...
            embed.add_field(
                name="Staff Role",
                value=f"<@&{settings['MOD_ROLE']}>"
                if settings["MOD_ROLE"]
                else "Not configured",
                inline=False,
            )
            embed.add_field(
                name="Delegated Setup Administrators",
        ...
        if resource_issues:
            embed.add_field(
                name="Resource Issues", value="\n".join(resource_issues), inline=False
            )
            embed.add_field(
                name="Recommended Action",
                value="Run `/setup repair` after resolving any missing permissions.",
                inline=False,
            )
    ...
    @setup_group.command(
        name="staff", description="Change the role that can access and manage tickets"
    )
    @app_commands.describe(staff_role="Role that should manage tickets")
    async def setup_staff(
        self, interaction: discord.Interaction, staff_role: discord.Role
    ):
        if not await self.require_setup_admin(interaction):
            return
        ...
        try:
            settings, missing_permissions = await self.configure_server(
                interaction.guild, staff_role, None
            )
        except discord.HTTPException as error:
            reference = log_exception(
                "ONBOARDING",
    ...
    @setup_group.command(
        name="tickets", description="Change the ticket types shown in the panel"
    )
    @app_commands.describe(
        ticket_types="Comma-separated ticket types, with a maximum of ten"
    )
    async def setup_tickets(
        self,
        interaction: discord.Interaction,
        ticket_types: app_commands.Range[str, 2, 500],
    ):
        if not await self.require_setup_admin(interaction):
            return
    ...
    @setup_group.command(
        name="admins", description="List members with delegated setup access"
    )
    async def setup_admins(self, interaction: discord.Interaction):
        ...
        value = (
            "\n".join(f"<@{user_id}> (`{user_id}`)" for user_id in admin_ids)
            if admin_ids
            else "No delegated setup administrators."
        )
        embed = discord.Embed(
    ...
    @add_group.command(
        name="admin", description="Allow a member to configure the ticket system"
    )
    @app_commands.describe(
        member="Select a server member by username",
        user_id="Alternatively enter the member's Discord user ID",
    )
    async def add_admin(
        self,
        interaction: discord.Interaction,
        ...
        embed.add_field(
            ...
            value=str(len(settings["SETUP_ADMIN_USERS"])),
            inline=False,
        )
        embed.set_footer(text=f"{config.BOT_NAME} | Access Management")
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    @remove_group.command(
        name="admin", description="Remove a member's delegated setup access"
    )
    @app_commands.describe(
        member="Select a server member by username",
        user_id="Alternatively enter the member's Discord user ID",
    ...
    @setup_group.command(
        name="repair",
        description="Recreate missing ticket resources and publish a fresh panel",
    )
    ...
    @setup_group.command(
        name="reset",
        description="Reset bot configuration without deleting Discord channels",
    )
    ...
    @app_commands.command(
        name="invite",
        description="Get the official bot installation link and permission summary",
    )
    async def invite_command(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Security",
            value="The installation does not request the Administrator permission.",
            inline=False,
        )
    ...
    @app_commands.command(
        name="privacy", description="View the bot's data and privacy summary"
    )
    async def privacy_command(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Ticket Content",
            value="Ticket messages are read when generating transcripts or evaluating inactivity. Transcript files are created only as part of ticket operations.",
            inline=False,
        )
        embed.add_field(
            name="Administrator Control",
            value="Server administrators can reset configuration with `/setup reset`. A public Privacy Policy and Terms of Service must be linked before App Directory release.",
            inline=False,
        )
        if config.PRIVACY_POLICY_URL:
            embed.add_field(
                name="Privacy Policy",
                value=f"[Read the full Privacy Policy]({config.PRIVACY_POLICY_URL})",
                inline=False,
            )
        if config.TERMS_OF_SERVICE_URL:
            embed.add_field(
                name="Terms of Service",
                value=f"[Read the Terms of Service]({config.TERMS_OF_SERVICE_URL})",
                inline=False,
            )
