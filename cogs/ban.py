...
async def resolve_banned_user(
    ...
    if guild:
        ...
        try:
            ban_entries = [
                entry async for entry in guild.bans()
            ]
...
class BanCog(commands.Cog):
    ...
    async def warnz_slash(
        ...
        if not can_warn_or_view_history(
            interaction.user
        ):
        ...
        if isinstance(
            ...
            try:
                ...
                dm_embed = discord.Embed(
                    ...
                    description=(
                        f"You have been issued a warning in "
        ...
        embed = discord.Embed(
            ...
            description=(
                f"An official warning has been registered "
                f"for **{target_name}**."
            ),
            color=discord.Color.from_rgb(
                241,
    ...
    async def warnremovez_slash(
        ...
        if count_removed == 0:
            ...
            if warn_id:
                ...
                message = (
                    f"No warning found with ID/UUID "
                ...
                message = (
                    f"No active warning records found for "
        ...
        if isinstance(
            ...
            try:
                ...
                dm_embed = discord.Embed(
                    ...
                    description=(
                        f"A warning issued on your account in "
                        f"**{interaction.guild.name}** "
                        "has been removed."
                    ),
                    color=discord.Color.from_rgb(
    ...
    async def build_history_embed(
        ...
        if (
            ...
            risk_color = discord.Color.from_rgb(
                231,
                76,
                60
            )
            ...
            warn_count >= 1
            or total_bad_words >= 3
        ):

            risk_status = (
                "[NOTICE - WARNINGS RECORDED]"
            )
    ...
    @app_commands.command(
        name="infraction",
        description=(
            "Search or manage an infraction "
            "by its unique UUID"
        )
    )
    @app_commands.describe(
        uuid="The unique UUID of the infraction to look up",
        action=(
    ...
    async def infraction_slash(
        ...
        if not infraction:
            ...
            await interaction.response.send_message(
                embed=error(
                    f"No infraction found matching UUID: `{uuid}`"
                ),
                ephemeral=True
            )
        ...
        elif infraction["action_type"] == "WARN":
    ...
    async def infraction_prefix(
        ...
        if not uuid_input:
            ...
            await ctx.send(
                embed=error(
                    "Please specify an Infraction UUID to search for."
                )
            )
        ...
        if not infraction:
            ...
            await ctx.send(
                embed=error(
                    f"No infraction found matching UUID: "
                    f"`{uuid_input}`"
                )
            )
