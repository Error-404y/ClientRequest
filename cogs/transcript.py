...
import shutil
import re
from utils.logger import log_transcript
...
async def download_file(url, destination):
    try:
        ...
        print(f"Failed to download asset {url} to {destination}: {str(e)}")
    return False

def parse_markdown(text):
    if not text:
        return ""
    # Escape HTML first to prevent any code/script injection
    ...
    # User / Role mentions: <@ID> or <@&ID> or <#ID>
    ...
    # Direct URLs: http://... (not inside href/src attributes)
...
async def create_transcript(channel, lightweight=False):
    ...
    async for message in channel.history(limit=None, oldest_first=True):
        ...
        # Retrieve member object to check roles
        member = guild.get_member(message.author.id)
        
        # Determine class for filtering
        msg_class = "msg-user"
        ...
        # Resolve badge
        badge_html = ""
        if message.author.id in config.OWNER_ROLES or message.author.id == config.SETUP_USER_ID:
        ...
        if not lightweight:
            ...
            if author_id not in downloaded_avatars:
                ...
                if success:
                    ...
                    downloaded_avatars[author_id] = avatar_url  # Fallback to CDN URL
        ...
        for attachment in message.attachments:
            ...
            # Check if it is an image to show inline preview
        ...
        for embed in message.embeds:
            ...
            # Parse simple markdown inside embed description if present
        ...
        # Avoid printing [No message content] if we have embeds or attachments
    ...
    # Discord-themed dark mode UI with interactive controls
    page = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ZER Ticket Transcript</title>
    <style>
        body {{
...
<body>
    <div class="container">
        ...
        <div class="footer">
            Generated by ZER Ticket System
        </div>
    </div>
...
"""
    ...
    try:
        ...
        print(f"Failed to delete temporary transcript dir: {str(e)}")
