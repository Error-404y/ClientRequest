...
class CloseTicketModal(discord.ui.Modal, title="Close Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Close the ticket channel using the shared actions helper
...
class PrioritySelectionView(discord.ui.View):
    ...
    async def update_priority(self, interaction: discord.Interaction, priority: str):
        ...
        # Edit channel name in background
        async def rename_bg():
            try:
...
class TicketButtons(View):
    ...
    async def claim(self, interaction, button):
        ...
        await claim_ticket(interaction.channel.id, interaction.user.id, claimed_at)

        # Update topic to keep track of claimed user
        channel = interaction.channel
        owner_id = None
        ...
        # Edit channel topic in background
        async def edit_topic_bg():
            try:
                await channel.edit(topic=channel_topic)
            except Exception:
                pass
        asyncio.create_task(edit_topic_bg())

        # Update the view: disable the claim button and change label
