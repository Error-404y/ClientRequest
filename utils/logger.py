...
SENSITIVE_PATTERNS = (
    re.compile(
        r"(?i)(discord_token|token|authorization|password|secret)(\s*[=:]\s*)([^\s,;]+)"
    ),
...
def _fallback(message):
    try:
        sys.stderr.write(
            f"{datetime.now(timezone).isoformat()} | LOGGER | {redact(message)}\n"
        )
        sys.stderr.flush()
    except Exception:
        return False
...
def redact(value):
    ...
    for pattern in SENSITIVE_PATTERNS:
        text = (
            pattern.sub(r"\1\2[REDACTED]", text)
            if pattern.groups >= 3
...
def _guild(value, accept_integer=False):
    if value is None:
        return None
    if isinstance(value, int):
        return value if accept_integer else None
...
class ReadableFormatter(logging.Formatter):
    def format(self, record):
        timestamp = datetime.fromtimestamp(record.created, timezone).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        return (
            f"{timestamp} | {level:<8} | {category:<12} | {record.getMessage()}{suffix}"
        )
...
class ConsoleFormatter(ReadableFormatter):
    def format(self, record):
        ...
        level = getattr(record, "display_level", record.levelname)
        return (
            f"{COLORS.get(level, '')}{rendered}{COLORS['RESET']}"
            if os.isatty(1)
            else rendered
        )
...
def create_error_fingerprint(category, error, context=None, guild_id=None):
    frames = traceback.extract_tb(error.__traceback__)
    location = "unknown"
    if frames:
        frame = frames[-1]
        location = f"{Path(frame.filename).name}:{frame.name}"
    raw = f"{category.upper()}|{type(error).__name__}|{context or ''}|{location}|guild={guild_id or 0}"
...
def _store_error(
    ...
    try:
        ...
        _fallback(
            f"Failed to store error event: {type(storage_error).__name__}: {storage_error}"
        )
        return reference
    return reference
...
def emit(level, category, message, guild=None, channel=None, user=None, reference=None):
    ...
    extra = {
        ...
        "reference": reference,
        "guild_id": _guild(guild, accept_integer=True)
        or _guild(channel)
        or _guild(user),
    ...
    if normalized in {"ERROR", "CRITICAL"}:
        ...
        try:
            ...
            _fallback(
                f"Failed to write {filename}: {type(storage_error).__name__}: {storage_error}"
            )


def log_exception(category, error, guild=None, channel=None, user=None, context=None):
    trace = "".join(traceback.format_exception(type(error), error, error.__traceback__))
    guild_id = _guild(guild, accept_integer=True) or _guild(channel) or _guild(user)
    fingerprint = create_error_fingerprint(category, error, context, guild_id)
    ...
    try:
        filename = (
            "test-tracebacks.log" if category.upper() == "TEST" else "tracebacks.log"
        )
        ...
        record = logging.LogRecord(
            ...
            "",
            0,
            f"{datetime.now(timezone).isoformat()} | {reference}\n{redact(trace)}",
            (),
            None,
        )
        handler.handle(record)
        ...
        _fallback(
            f"Failed to write traceback log: {type(storage_error).__name__}: {storage_error}"
        )
    return reference
        ...
        with sqlite3.connect(config.DATABASE, timeout=2) as database:
            database.execute(
                ...
                (
                    ...
                    duration,
                    threshold_ms,
                    _guild(guild, accept_integer=True),
                    datetime.now(timezone).isoformat(),
                ),
            )
    except (sqlite3.Error, OSError) as storage_error:
        _fallback(
            f"Failed to store performance event: {type(storage_error).__name__}: {storage_error}"
        )
    emit(
        "WARNING",
        "PERFORMANCE",
        f"Slow operation | {operation} | {duration:.1f} ms | limit {threshold_ms:.1f} ms",
        guild=guild,
    )
    return duration
...
def log_dm(recipient, subject, success=True, error_detail=None):
    emit(
        ...
        "DM",
        f"{subject} | {format_user(recipient)}{f' | {error_detail}' if error_detail else ''}",
        user=recipient,
    )
...
def log_mod(action, moderator, target, reason=None, extra=None):
    details = [
        action,
        f"moderator={format_user(moderator)}",
        f"target={format_user(target)}",
    ]
    if reason:
...
def log_command(user, command_name, channel=None, details=None):
    emit(
        "INFO",
        "COMMAND",
        f"{format_user(user)} used {command_name}{f' | {details}' if details else ''}",
        channel=channel,
        user=user,
    )
...
def log_ticket(action, channel, user=None, details=None):
    ...
    emit(
        level,
        "TICKET",
        f"{action} | {format_channel(channel)}{f' | {details}' if details else ''}",
        channel=channel if not isinstance(channel, str) else None,
        user=user,
    )
...
def log_interaction(user, custom_id, channel=None, details=None):
    emit(
        "INFO",
        "INTERACTION",
        f"{format_user(user)} triggered {custom_id}{f' | {details}' if details else ''}",
        channel=channel,
        user=user,
    )
...
def log_db(operation, table, details=None):
    emit(
        "DEBUG",
        "DATABASE",
        f"{operation} | {table}{f' | {details}' if details else ''}",
    )
...
def log_inactivity(action, channel=None, user=None, details=None):
    emit(
        "WARNING" if "fail" in action.lower() else "INFO",
        "INACTIVITY",
        f"{action}{f' | {details}' if details else ''}",
        channel=channel,
        user=user,
...
def ticket_close_report(channel, moderator, owner_id, reason, transcript_path, bot):
    log_ticket(
        ...
        channel,
        moderator,
        f"reason={reason} | applicant={owner_id or 'Unknown'} | transcript={transcript_path or 'Unavailable'}",
    )
