...
async def close_ticket_channel(channel, moderator, reason, bot):
    ...
    # Retrieve applicant (owner) ID safely from topic
    user_id = None
    ...
    # Database fallback for retrieving applicant ID
    if user_id is None:
        try:
    ...
    # Generate transcript first while user still has access
    ...
    # DM applicant with close reason and transcript zip
    dm_success = True
    if member:
        try:
            dm_content = (
    ...
    # Remove owner permissions safely
    if member:
        try:
    ...
    # Send close audit embed and attach transcript
    try:
        if zip_path and os.path.exists(zip_path):
    ...
    # Perform channel category move, rename, and send controls in a non-blocking background task
    async def perform_background_close_tasks():
        ...
        # Send closed ticket controls (reopen, transcript, delete)
        try:
            await channel.send(
    ...
    asyncio.create_task(perform_background_close_tasks())
